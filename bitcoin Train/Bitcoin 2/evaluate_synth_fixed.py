...
def load_synth_scaled():
    ...
    # Convert inverse-scaled synth back to scaled using scalers fitted on train
    with open(os.path.join(DATA_DIR, "scalers.pkl"), "rb") as f:
        scalers = pickle.load(f)  # This is a list of scalers, one per feature
    
    n, T, D = synth_inv.shape
    synth_scaled = np.zeros_like(synth_inv)
    
    # Apply each scaler to its corresponding feature dimension
...
def crypto_specific_metrics(real_data, synth_data):
    """Bitcoin-specific quality metrics"""
    metrics = {}
    
    # Volatility clustering check
    real_vol = np.std(real_data, axis=1)
    synth_vol = np.std(synth_data, axis=1)
    min_len = min(len(real_vol), len(synth_vol))
    metrics['volatility_similarity'] = np.corrcoef(real_vol[:min_len], synth_vol[:min_len])[0,1]
    
    # Fat-tailed distribution check
    real_flat = real_data.flatten()
    synth_flat = synth_data.flatten()
    real_kurtosis = np.mean((real_flat - np.mean(real_flat))**4) / (np.var(real_flat)**2)
    synth_kurtosis = np.mean((synth_flat - np.mean(synth_flat))**4) / (np.var(synth_flat)**2)
    metrics['kurtosis_ratio'] = synth_kurtosis / (real_kurtosis + 1e-8)
...
def run():
    ...
    # ADD CRYPTO-SPECIFIC METRICS HERE
    crypto_metrics = crypto_specific_metrics(real_samp, synth_samp)
    out.update(crypto_metrics)
    print(f"Crypto metrics - Volatility similarity: {crypto_metrics['volatility_similarity']:.4f}, Kurtosis ratio: {crypto_metrics['kurtosis_ratio']:.4f}")
